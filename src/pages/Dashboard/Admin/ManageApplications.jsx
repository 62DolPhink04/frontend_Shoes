...
const ManageApplications = () => {
  ...
  useEffect(() => {
    fetch("https://backend-shoes-79qb.onrender.com/applied-instrustion/all")
      .then((res) => res.json())
  ...
  const handleStatusChange = async (id, newStatus, email) => {
    try {
      ...
      const response = await fetch(
        `https://backend-shoes-79qb.onrender.com/update-instructor-status/${id}`,
        {
          method: "PATCH",
          headers: {
      ...
      if (result.modifiedCount > 0) {
        if (newStatus === "approved") {
          ...
          await axios.patch(
            `https://backend-shoes-79qb.onrender.com/promote-to-instructor/${email}`
          );
        }
  ...
  const denyInstructor = async (id, email) => {
    try {
      const response = await axios.patch(
        `https://backend-shoes-79qb.onrender.com/deny-instructor/${email}`
      );
